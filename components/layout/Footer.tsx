  ...
  return (
    <footer className="border-t border-fg/10 bg-bg">
      <div className="mx-auto max-w-[1400px] px-6 md:px-10">
        ...
        <div className="flex flex-col gap-8 border-b border-fg/8 py-10 md:flex-row md:items-center md:justify-between">
          {/* Logo */}
          ...
          <nav className="flex items-center gap-6 md:gap-8">
            {navLinks.map(({ href, label }) => (
              <Link
                key={href}
                href={href}
                className="font-body text-[10px] font-medium uppercase tracking-widest text-fg/45 transition-colors duration-200 hover:text-fg"
              >
                {label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map(({ href, label, Icon }) => (
              <a
                ...
                aria-label={label}
                className="text-fg/40 transition-colors duration-200 hover:text-fg"
              >
                <Icon size={15} />
              </a>
            ))}
        ...
        {/* Row 2: Location · Copyright */}
        <div className="flex flex-col gap-2 py-5 md:flex-row md:items-center md:justify-between">
          <p className="font-body text-[10px] font-medium uppercase tracking-widest text-fg/35">
            Lagos, Nigeria
          </p>
          <p className="font-body text-[10px] font-medium uppercase tracking-widest text-fg/35">
